...
Rajya Sabha counterpart to merge_into_projects_v4.py. Identical logic,
line for line — the only difference is which raw_landing source labels
it reads (rajyasabha_* instead of esakshi_*).

Writes into the SAME core tables as the LS merge script:
  dim_mp, dim_ida, dim_vendor, fact_projects, fact_expenditures

This is deliberately safe to run either before or after
merge_into_projects_v4.py, in any order, any number of times:
  - dim_mp / dim_ida / dim_vendor use ON CONFLICT DO UPDATE / DO NOTHING
  - fact_projects uses ON CONFLICT (work_id) DO UPDATE
  - RS work IDs follow the same WS/MP.../... scheme as LS but with a
    different MP-number range, so they will never collide with LS work
    IDs already in fact_projects.

Run this AFTER automated_ingestion_rajya_sabha.py has populated
raw_landing with the rajyasabha_* rows.
"""
...
# ============================================================
# HELPER FUNCTIONS (identical to merge_into_projects_v4.py)
# ============================================================
...
# ============================================================
# DATABASE HELPERS (identical to merge_into_projects_v4.py)
# ============================================================
...
def main():
    ...
    try:
        for work_id, stages in sorted(grouped.items()):
            ...
            # Same final_amount definition as the LS merge script - see the
            # comment there, and task #3 (team sign-off), for the reasoning.
            final_amount = sanctioned_amount if sanctioned_amount is not None else recommended_amount
