...
def main():
    ...
    grouped = {}
    for raw_id, payload in {**recommended, **sanctioned, **completed, **expenditure}.items():
        dtl_id = payload.get("WORK_RECOMMENDATION_DTL_ID")
        resolved_id = f"DTL-{dtl_id}" if dtl_id else raw_id
        grouped.setdefault(resolved_id, {"rec": {}, "sanc": {}, "comp": {}, "exp": {}})

    for raw_id, payload in recommended.items():
        dtl_id = payload.get("WORK_RECOMMENDATION_DTL_ID")
        key = f"DTL-{dtl_id}" if dtl_id else raw_id
        grouped[key]["rec"] = payload
    for raw_id, payload in sanctioned.items():
        dtl_id = payload.get("WORK_RECOMMENDATION_DTL_ID")
        key = f"DTL-{dtl_id}" if dtl_id else raw_id
        grouped[key]["sanc"] = payload
    for raw_id, payload in completed.items():
        dtl_id = payload.get("WORK_RECOMMENDATION_DTL_ID")
        key = f"DTL-{dtl_id}" if dtl_id else raw_id
        grouped[key]["comp"] = payload
    for raw_id, payload in expenditure.items():
        dtl_id = payload.get("WORK_RECOMMENDATION_DTL_ID")
        key = f"DTL-{dtl_id}" if dtl_id else raw_id
        grouped[key]["exp"] = payload
